...
import numpy as np
import torch
from algorithms.Gaussian.scene import Scene
import os
from tqdm import tqdm
from os import makedirs
from algorithms.Gaussian.gaussian_renderer import render
import torchvision
from algorithms.Gaussian.utils.general_utils import safe_state
from argparse import ArgumentParser
from algorithms.Gaussian.arguments import ModelParams, PipelineParams, get_combined_args, ModelHiddenParams
from algorithms.Gaussian.gaussian_renderer import GaussianModel
from time import time
import open3d as o3d
from algorithms.Gaussian.utils.graphics_utils import fov2focal
import cv2
...
def render_set(project_id, model_path, name, iteration, views, gaussians, pipeline, background, no_fine, render_test=False, reconstruct=False):
    ...
    imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(iteration), 'ours_video.mp4'), 
                 render_array, fps=30, quality=8, format='FFMPEG')
    ...
    if reconstruct:
        reconstruct_point_cloud(project_id, render_images, mask_list, render_depths, camera_parameters, name)

def render_sets(project_id, dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool, reconstruct: bool):
    import psutil
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(project_id, dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, dataset.no_fine, reconstruct=False)
        if not skip_test:
            render_set(project_id, dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, dataset.no_fine, reconstruct=reconstruct)
        if not skip_video:
            render_set(project_id, dataset.model_path,"video",scene.loaded_iter, scene.getVideoCameras(),gaussians,pipeline,background, dataset.no_fine, render_test=True, reconstruct=False)

def reconstruct_point_cloud(project_id, images, masks, depths, camera_parameters, name):
    import cv2
    import copy
    import config
    output_frame_folder = os.path.join(config.RECONSTRUCT_DIR, f'{project_id}')
