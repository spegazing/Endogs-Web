...
def get_combined_args(parser: ArgumentParser, input_args=None):
    """
    合并参数：从配置文件读取参数，并用传入的参数覆盖
    
    Args:
        parser: ArgumentParser 对象
        input_args: 传入的参数对象（可以是 Namespace、字典或 None）
    ...
    if input_args is None:
        ...
        args_input = parser.parse_args(cmdlne_string)
    elif isinstance(input_args, Namespace):
        # 如果传入的是 Namespace 对象
        args_input = input_args
    elif isinstance(input_args, dict):
        # 如果传入的是字典，转换为 Namespace
        args_input = Namespace(**input_args)
    else:
        # 默认情况
        args_input = Namespace()

    # 2. 尝试从模型路径下的 cfg_args 文件读取配置
    cfgfile_string = "Namespace()"
    try:
        # 构造配置文件路径：model_path/cfg_args
        if hasattr(args_input, 'model_path') and args_input.model_path:
            cfgfilepath = os.path.join(args_input.model_path, "cfg_args")
    ...
    # 4. 合并两个来源的参数
    merged_dict = vars(args_cfgfile).copy()  # 从配置文件复制参数
    
    # 5. 传入的参数覆盖配置文件参数
    for k, v in vars(args_input).items():
        if v is not None:  # 如果传入了该参数（不为None）
            merged_dict[k] = v

    # 6. 返回合并后的 Namespace 对象
