...
import torch
from random import randint
from algorithms.Gaussian.utils.loss_utils import l1_loss, ssim, lpips_loss, TV_loss
from algorithms.Gaussian.gaussian_renderer import render, network_gui
import sys
from algorithms.Gaussian.scene import Scene, GaussianModel
from algorithms.Gaussian.utils.general_utils import safe_state
import uuid
from tqdm import tqdm
from algorithms.Gaussian.utils.image_utils import psnr
...
from algorithms.Gaussian.utils.timer import Timer
from torchmetrics.functional.regression import pearson_corrcoef

import lpips
from algorithms.Gaussian.utils.scene_utils import render_training_image
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    first_iter = 0                                           # 初始迭代次数，从0开始
    gaussians.training_setup(opt)                            # 设置高斯模型训练参数
    if checkpoint:
        (model_params, first_iter) = torch.load(checkpoint)  # 加载模型参数
        gaussians.restore(model_params, opt)                 

    bg_color = [1, 1, 1] if dataset.white_background else [0, 0, 0] # 背景颜色
    background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

    iter_start = torch.cuda.Event(enable_timing = True)        # 迭代开始时间
    iter_end = torch.cuda.Event(enable_timing = True)          # 迭代结束时间

    viewpoint_stack = None                                    # 视角栈
    ema_loss_for_log = 0.0                                    # 平均损失
    ema_psnr_for_log = 0.0                                    # 平均PSNR
    
    final_iter = train_iter                                    # 最终迭代次数
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        if stage == 'coarse':  
            ...
            idx = randint(0, len(viewpoint_stack)-1)      # 细阶段，随机选择一个视角
        viewpoint_cams = [viewpoint_stack[idx]]

        # Render
        if (iteration - 1) == debug_from:
            pipe.debug = True            # 调试模式
        ...
        radii_list = []                                     # 半径列表
        visibility_filter_list = []                         # 可见性过滤列表
        viewspace_point_tensor_list = []                    # 视空间点张量列表
...
def training(args, dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname, extra_mark):
    tb_writer = prepare_output_and_logger(args, expname)
    gaussians = GaussianModel(dataset.sh_degree, hyper)   # create GaussianModel
    dataset.model_path = args.model_path                  # set model_path to args.model_path
    timer = Timer()
    scene = Scene(dataset, gaussians, load_coarse=args.no_fine) # create Scene
    timer.start()
    # 更新配置
    import config
    opt.coarse_iterations = config.User_coarse_iterations
    opt.iterations = config.User_iterations
    opt.percent_dense = config.User_percent_dense
    opt.opacity_reset_interval = config.User_iterations
    print(f'opt.coarse_iterations：{opt.coarse_iterations}')
    print(f'opt.iterations：{opt.iterations}')
    print(f'opt.percent_dense：{opt.percent_dense}')
    print(f'opt.opacity_reset：{config.User_iterations}')
    # 重建
